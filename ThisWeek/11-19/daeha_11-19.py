...
@Probability_box_upgrade.register  #TODO(): ABC를 통해서 class를 만들때만 가능?
...
def main():
    ...
    real_subclasses = Probability_box_upgrade.__subclasses__
    virtual_subclasses = Probability_box_upgrade._abc_registry
    
    for r, v in zip([real_subclasses], [virtual_subclasses]):
        test(r, v)
...
def test(r, v, verbose=False):
    print('virtual_subclasses is {}'.format(v.data))
    
    
if __name__ == "__main__":
    main()  # virtual_subclasses if {<weakref at ---; to 'ABCMeta' at --->}
            # 0
            # 10
            # 20